
def RemoveDiacriticsAndSpaces(inputString):

    replaceTable = {" ":"","ß":"ss","à":"a","á":"a","â":"a","ã":"a","ä":"a","å":"a","æ":"ae","ç":"c","è":"e","é":"e","ê":"e","ë":"e","ì":"i","í":"i","î":"i","ï":"i","ð":"d","ñ":"n","ò":"o","ó":"o","ô":"o","õ":"o","ö":"o","ø":"o","ù":"u","ú":"u","û":"u","ü":"u","ý":"y","þ":"p","ÿ":"y"}

    for key in replaceTable.keys():
        inputString = inputString.replace(key,replaceTable[key])
    
    return inputString

def ParseLocation(idLocation,locationsList):
    try:
        id = list(filter(lambda x: locationsList[x].idLocation == idLocation, locationsList))[0]
    except:
        return 10
    return id

